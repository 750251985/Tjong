...
class myModel(nn.Module):
    def __init__(self, card_feat_depth, num_extra_feats, num_cards, num_actions):
        ...
        self.card_net = nn.Sequential(
            nn.Linear(linear_length, hidden_layers_size[0]),
            nn.ReLU(),
            nn.Linear(hidden_layers_size[0], hidden_layers_size[1]),
            nn.ReLU(),
            nn.Linear(hidden_layers_size[1], linear_length),
            nn.ReLU(),
        )
    ...
    def forward(self, card_feats, extra_feats, device, decide_which, mask):
        ...
        card_feats = torch.reshape(card_feats,(card_feats.shape[0],card_feats.shape[1],card_feats.shape[2]*card_feats.shape[3]))
                ...
                if rand > 90:
                    ...
                    for kind in ['play', 'action', 'chi_gang']:
                        if len(training_data[kind]['card_feats']) >= self.batch_size:
                            data = training_data[kind]
                            # flops, params = profile(self.model, inputs=(np.array(data['card_feats']),
...
# nohup python -u DL_MLP_3heads_agent.py > log_MLP_3heads_04120107.log 2>&1 &