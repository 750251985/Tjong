...
parser.add_argument('-lp', '--load_path', type=str, default='./models/dl_TIT_3heads_blocks3_fc3_1680715269.823202.pt', help='from where to load model')
...
class myModel(nn.Module):
    def __init__(self, card_feat_depth, num_extra_feats, num_cards, num_actions):
        ...
        linear_length = num_cards * card_feat_depth
        self.card_net = nn.GRU(input_size=linear_length, hidden_size=hidden_layers_size[0], num_layers=6, batch_first= True)
        self.card_play_decision_net = nn.Sequential(
            nn.Linear(hidden_layers_size[0]+num_extra_feats, hidden_layers_size[1]),
            nn.ReLU(),
            nn.Linear(hidden_layers_size[1], hidden_layers_size[0]),
            nn.ReLU(),
        ...
        self.action_decision_net = nn.Sequential(
            ...
            nn.Linear(hidden_layers_size[1], hidden_layers_size[0]),
            nn.ReLU(),
            nn.Linear(hidden_layers_size[0], num_actions),
        )
    ...
    def forward(self, card_feats, extra_feats, device, decide_which, mask):
        ...
        card_layer,_ = self.card_net(card_feats)
        ...
        if self.train and not self.botzone and not initial and self.round_count % self.print_interval == 0:
            ...
            with torch.no_grad():
                ...
                for kind in ['play', 'action', 'chi_gang']:
                    ...
                    start = 0
                    flag = True
                    it = iter(range(512, len(data["target"]), 512))
                    while flag:
                        x = next(it, len(data["target"]))
                        if len(data["card_feats"][start:x]) > 0:
                            probs_temp = self.model(np.array(data['card_feats'][start:x]),
                                               np.array(data['extra_feats'][start:x]), self.device,
                                                    kind, np.array(data['mask'][start:x]))
                            start = x
    ...
    def step(self, request=None, response_target=None, fname=None):
        ...
        if self.turnID <= 1:
            ...
            action_probs = self.model(np.array([card_feats]),
                                      np.array([extra_feats]), self.device, 'action', available_action_mask)
            ...
            if responses(action) in [responses.CHI, responses.ANGANG, responses.BUGANG]:
                card_probs = self.model(np.array([card_feats]),
                                        np.array([extra_feats]), self.device, 'chi_gang', available_card_mask[action].numpy())
            ...
            if responses(action) in [responses.PLAY, responses.CHI, responses.PENG]:
                ...
                card_probs = self.model(np.array([card_feats]),
                                        np.array([extra_feats]), self.device, 'play', np.array(card_mask))
...
# nohup python -u DL_RNN_3heads_agent.py > log_DL_RNN_3heads_04210001.log 2>&1 &