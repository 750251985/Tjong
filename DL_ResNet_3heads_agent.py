...
parser.add_argument('-lp', '--load_path', type=str, default='./models/', help='from where to load model')
...
args.model_name = 'dl_ResNet_3heads'
args.cuda = 'cuda:1'
...
class myModel(nn.Module):
    def __init__(self, card_feat_depth, num_extra_feats, num_cards, num_actions):
        ...
        self.card_net = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        # 修改全连接层的输出
        new_conv = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.card_net.conv1 = new_conv
        out_features = self.card_net.fc.out_features
        self.card_play_decision_net = nn.Sequential(
            nn.Linear(num_extra_feats + out_features, hidden_layers_size[0]),
            nn.Sigmoid(),
        ...
        self.action_decision_net = nn.Sequential(
            nn.Linear(num_extra_feats + out_features, hidden_layers_size[1]),
            nn.ReLU(),
...
# nohup python -u DL_ResNet_3heads_agent.py > log_ResNet_3heads_04120033.log 2>&1 &
