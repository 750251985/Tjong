...
# from TIT_3heads import Config_3heads, TIT_3heads
...
parser.add_argument('-cp', '--critic_path', type=str,
                    default='./models/dl_TIT_3heads_blocks3_fc3_1681075761.7407126.pt',
                    help='path to load critic model')
...
parser.add_argument('-tn', '--total_number', type=int, default=50000, help='epochs to run in total，全部epochs，控制停止')
parser.add_argument('-te', '--test_epochs', type=int, default=2, help='test epochs to run，一次测试循环次数')
parser.add_argument('-rn', '--round_number', type=int, default=4,
                    help='round number*repeated_times to run in parallel，一个round跑几圈')
parser.add_argument('-rt', '--repeated_times', type=int, default=2,
...
parser.add_argument('-lp', '--log_path', type=str, default='none', help='log path, set to "none" for no logging')
parser.add_argument('-bs', '--batch_size', type=int, default=512, help='max batch size to update model')
parser.add_argument('-ga', '--gamma', type=float, default=0.96, help='cumulate reward discount factor')
args = parser.parse_args()
args.model_name = 'rl_ppo_r3'
args.cuda = 'cuda:1'
args.ps = '增加shanten'
...
class traj:
    ...
    def add_traj(self, kind=None, card_feats=None, extra_feats=None, mask=None,
                 card_feats_=None, extra_feats_=None, mask_=None, action=None,
                   logprob=None, turnID=None, value=None, value_=None, dw=None, reward=None):
        if kind is not None:
        ...
        if card_feats_ is not None:
            self.states_['card_feats'].append(card_feats_)
            self.states_['extra_feats'].append(extra_feats_)
            self.states_['masks'].append(mask_)
    ...
    def form_traj(self):
        for key, val in self.states.items():
            self.states_[key] = self.states[key][1:]
            if len(self.states[key]) > 0:
                self.states_[key].append(np.zeros((self.states[key][0].shape)))  # 补充最后一个状态
        self.values_ = self.values[1:]
        if len(self.values) > 0:
            self.values_.append(np.array([1.0]))
        self.dw.extend([0] * len(self.actions))
        self.dw[-1] = 1  # 最后一步本盘结束

    def merge_memory(self, traj):
        self.kinds.extend(traj.kinds)
        for key, val in self.states.items():
            val.extend(traj.states[key])
            self.states_[key].extend(traj.states_[key])
        self.actions.extend(traj.actions)
        self.logprobs.extend(traj.logprobs)
        self.turnIDs.extend(traj.turnIDs)
        self.values.extend(traj.values)
        self.values_.extend(traj.values_)
        self.dw.extend(traj.dw)
...
class RoundMemory:  # collected from old policy
    def __init__(self):
        self.roundMemory = traj()

    def get_RoundMemory(self):
        return self.roundMemory

    def add_traj(self, kind=None, card_feats=None, extra_feats=None, mask=None, action=None, logprob=None,
                   turnID=None, value=None, reward=None):
        self.roundMemory.add_traj(kind=kind, card_feats=card_feats, extra_feats=extra_feats, mask=mask,
                                  action=action, logprob=logprob, turnID=turnID, value=value, reward=reward)

    def add_reward(self, rewards=None):
        self.roundMemory.add_rewards(rewards)
        self.roundMemory.form_traj()

    def clear_memory(self):
        self.roundMemory.clear_memory()
...
class Memory:  # collected from old policy
    ...
    def merge_round_memory(self, round_memory):
        m = round_memory.get_RoundMemory()
        for kind, card_feats, extra_feats, mask, card_feats_, extra_feats_, mask_, actions, logprob, turnIDs, value, value_, dw, reward in \
                zip(m.kinds, m.states['card_feats'], m.states['extra_feats'], m.states['masks'], m.states_['card_feats'],
                    m.states_['extra_feats'], m.states_['masks'], m.actions, m.logprobs, m.turnIDs, m.values, m.values_, m.dw, m.rewards):
            self.memory[kind].add_traj(kind, card_feats, extra_feats, mask, card_feats_, extra_feats_, mask_, actions, logprob, turnIDs, value, value_, dw, reward)

    def merge_memory(self, local_memory):
        for kind, m in local_memory.memory.items():
            self.memory[kind].merge_memory(m)
...
class Critic(nn.Module):
    def __init__(self,obs_array_dim=22 * 34, num_extra_feats=25, num_cards=34, num_actions=8):
        super().__init__()
        self.entropy_weight = args.entropy_weight
        activation_fn = {'tanh': nn.Tanh(), 'relu': nn.ReLU(), 'gelu': nn.GELU()}
        self.ln1_value = nn.LayerNorm(obs_array_dim+num_extra_feats)
        self.head_value = nn.Sequential(
            nn.Linear(obs_array_dim+num_extra_feats, 1024 * 2),
            activation_fn['relu'],
...
class Policy(nn.Module):
    ...
    def forward(self, card_feats, extra_feats, device, decide_which):
        ...
        obs = np.concatenate((card_feats, extra_feats), axis=1)
        obs_tensor = torch.from_numpy(obs).to(device).to(torch.float32)
        probs = self.softmax(self.tit(obs_tensor, decide_which))
        return probs
...
class MahjongEnv:
    def __init__(self, old_model_path=None, shared_model=None, shared_critic=None, cuda='cpu', shared_memory=None, lock=None):
        ...
        self.memory_each_rounds = [RoundMemory() for _ in range(4 * self.round_number)]
        self.device = torch.device(cuda if use_cuda else "cpu")
        print('using ' + str(self.device))
        self.total_cards = 34
        self.total_actions = len(responses) - 2
        self.model = Policy().to(self.device)
        self.critic = Policy().to(self.device)
        self.old_model = Policy().to(self.device)
        self.shared_critic = shared_critic.to(self.device)
        self.shared_critic.eval()
        ...
        self.losers = np.zeros(4, dtype=int)
        self.win_steps = []
        self.losses = []
    ...
    def reset(self, initial=False):
        ...
        tempscores = self.scores4count
        tempscores[tempscores < 0] = 0
        self.avg_payoff = np.sum(tempscores, axis=0)[0] / (self.winners[0] + 1e-5)
                    ...
                    if requests[0] in ['hu', 'huangzhuang']:
                        ...
                        if outcome == 'hu':
                            ...
                            if dianpaoer is not None:
                                ...
                                self.losers[self.getBotInd(round_id, dianpaoer)] += 1
            ...
            for j in range(4):
                if self.train:
                    if self.scores[i][j] != 0 and self.getBotInd(i, j) in self.new_model_pos:
                        round_memory = self.memory_each_rounds[i * 4 + j].get_RoundMemory()
                        rewards = self.reshape_reward(round_memory, self.bots_orders[i], j,
                                                      self.scores[i], self.hand_fixed_data_bots_orders[i],
                                                      self.avg_payoff)
                        self.memory_each_rounds[i * 4 + j].add_reward(rewards)
                        self.local_memory.merge_round_memory(self.memory_each_rounds[i * 4 + j])
    ...
    def step(self, state, order, model, turnID):
        ...
        def run_forward(kind, turnID):
            ...
            for i in range(batch_number):
                ...
                value = self.shared_critic(card_feats[start:end],
                    extra_feats[start:end],
                    self.device,
                    kind)
            ...
            if self.train and model != self.old_model:
                for idx, action, log_prob, card_feat, extra_feat, mask, value in zip(action_idx, action_numpy,
                                                                                     log_probs,
                    ...
                    self.memory_each_rounds[original_order].add_traj(kind, card_feat, extra_feat, mask, action,
                                                                       log_prob, turnID, value)
    ...
    def reshape_reward(self, traj, bots, botInd, payoffs, hand_fixed_data_bots_order_round, avg_payoff):
        shantens = []
        usefuls = []
        aleady_in_hand = []
        ...
        for kind, card_feats, action in zip(traj.kinds, traj.states["card_feats"], traj.actions):
            # if winner == botInd and payoffs[winner] > 0:  # and payoffs[winner] > avg_payoff:
            card_feats = card_feats.reshape(22, 34)
            card = (card_feats[0, :] + card_feats[2, :]).astype(int)
            card_free = (card_feats[0, :]).astype(int)
            if winner == botInd:
                aleady_in_hand.append(sum(card & winnercard))
                if kind == "action" and int(action) == responses.HU.value:
                    shantens.append(0)
                    continue
            hand = []
            if kind =="play":
                card_free[action] = card_free[action] - 1
            if (kind=="action" and int(action) in [responses.PLAY.value,responses.MINGGANG.value,
                                                                    responses.ANGANG.value,responses.BUGANG.value]):
                shantens.append(shantens[-1])
                continue
            for ind, cardcnt in enumerate(card_free):
                for _ in range(cardcnt):
                    hand.append(self.getCardName(ind))
            if len(hand) not in [1, 4, 7, 10, 13]:
                print("ERROR_INVALID_HAND")
            (shanten, useful) = RegularShanten(hand=tuple(hand))
            shantens.append(shanten)
        if winner == botInd:
            shantens = np.array(aleady_in_hand) + np.array(shantens) -13
        x0 = shantens[:-1]
        x1 = shantens[1:]
        r_np = np.array(x0) - np.array(x1)
        rewards = r_np.tolist()
        if winner == botInd:
            rewards.append((payoffs[winner] / 2) ** 0.5)
        else:
            rewards.append(-((payoffs[winner] / 2) ** 0.5))
            # elif diaopao == botInd and payoffs[winner] > 0:
            #     if len(traj.actions) > 0:
            #         rewards.extend([0] * len(traj.actions))
            #         rewards[-1] = -((-payoffs[botInd] / 2) ** 0.5)
            # else:
            #     rewards.extend([0] * len(traj.actions))
        ...
        # if self.improve:
        #     if winner == botInd and payoffs[winner] > 0 and payoffs[winner] > avg_payoff:
        #         rewards.extend([(payoffs[botInd] / 2) ** 0.5] * len(m.actions))
        #     elif payoffs[botInd] < 0:
        #         if len(m.actions) > 0:
        #             rewards.extend([0] * len(m.actions))
        #             rewards[-1] = -((-payoffs[botInd] / 2) ** 0.5)
        #     else:
        #         rewards.extend([0] * len(m.actions))
        # else:
    ...
    def print_log(self, type, current_round, print_interval, winners, losers, player_scores, time_cost):
        win_sum = sum(winners)
        ...
        print(
            ...
            'Hu {} rounds，Huang-zhuang {} rounds，hu ratio {:.2%}, average rounds to hu: {}, took {:.2f} minutes per 10000 rounds'.format(
                type, total_rounds, rounds_this_stage + 1e-9,
                                    sum(winners[0::4]) / rounds_this_stage + 1e-9,
                                    sum(winners[1::1]) / rounds_this_stage + 1e-9, win_sum,
...
class MahjongHandler:
    ...
    def step(self, request):  # 获取当前state
        ...
        if self.turnID <= 1:
            ...
            extra_feats = np.concatenate((self.player_angang[1:], available_action_mask,
                                          ...
                                          self.tile_count, [self.turnID]))
            if extra_feats.shape[0] != 25:
                print("!!!!")
            self.prev_request = request
            self.turnID += 1
    ...
    def simulate_chi_peng(self, request, response, chi_peng_ind):
        ...
        extra_feats = np.concatenate((self.player_angang[1:], [my_free.sum()], action_mask,
                                      *np.eye(4)[[self.quan, self.myPlayerID]], self.tile_count, [self.turnID]))
...
class PPO_discrete(object):
    ...
    def update(self, all_memories: dict):
        ...
        for kind, memory in all_memories.items():
            ...
            vs = torch.from_numpy(np.array(memory.values)).to(self.device)
            vs_ = torch.from_numpy(np.array(memory.values_)).to(self.device)
            dw = torch.from_numpy(np.array(memory.dw)).to(self.device)
            ...
            for _ in range(self.K_epochs):
                ...
                if feature_size>1:
                    ...
                    card_feats, extra_feats, a, td_target, adv, old_logprobs = card_feats[perm], extra_feats[perm], \
                                                                                old_actions[perm], td_target[perm], adv[perm], old_logprobs[perm]
                ...
                for i in range(batch_number):
                    ...
                    ratio = torch.exp(new_logprobs - old_logprobs[start:end])  # a/b == exp(log(a)-log(b))
                    ...
                    '''critic update'''
                    c_loss = (new_value - td_target[start:end]).pow(2).mean()
...
# thread for training
def train_thread(cuda, shared_model, shared_critic, global_episode_counter, global_training_counter,
                 optimizer, critic_optimizer, lock, shared_memory):
    ...
    env = MahjongEnv(old_model_path=old_model_path,
                     ...
                     lock=lock)
    ppo = PPO_discrete(actor=shared_model, critic=shared_critic, actor_optimizer=optimizer, critic_optimizer=critic_optimizer, device=cuda)
    ...
    while global_episode_counter.value < args.total_number:
        ...
        with lock:
            ...
            for i in range(4):
                ...
                losers[i] += env.losers[i]
                player_scores[i] += np.sum(env.scores4count, axis=0)[i]
            avg_payoffs.append(env.avg_payoff)
            env.winners = np.zeros(4, dtype=int)
            env.losers = np.zeros(4, dtype=int)
            ...
            if current_round % print_interval == 0:
                ...
                env.print_log('train', current_round, print_interval, winners, losers, player_scores, time_cost)
                print(avg_payoffs)
                avg_payoffs = []
                for i in range(4):
                    winners[i] = 0
            ...
            if test and current_round % test_interval == 0:
                ...
                env.print_log('test', args.test_epochs, args.test_epochs, winners, losers, player_scores_test,
                              time_cost)
                if max(player_scores_test) == player_scores_test[0] and max(winners) == winners[0]:
                    env.improve = True
                else:
                    env.improve = False
                print("reward mode:", env.improve)
                for i in range(4):
                    winners[i] = 0
                    losers[i] = 0
                env.repeated_times = temp
                env.reset(False)
                env.train = True
...
def main():
    ...
    critic_optimizer = SharedAdam(shared_model.critic.parameters(), lr=lr)
    ...
    # losers_count = mp.Array('i', 4, lock=True)
    ...
    train_thread(args.cuda, shared_model, shared_critic, global_episode_counter,
                 global_training_counter, optimizer, critic_optimizer, lock, shared_memory)
...
# nohup python -u PPO_3heads.py > log_PPO_3heads_07200207.log 2>&1 &
