...
parser.add_argument('-si', '--save_interval', type=int, default=200, help='how often to save')
parser.add_argument('-ti', '--train_interval', type=int, default=10, help='how often to train backward')
parser.add_argument('-testi', '--test_interval', type=int, default=100, help='how often to test backward')
parser.add_argument('-ji', '--join_interval', type=int, default=100, help='how often to update shared model')
...
parser.add_argument('-rn', '--round_number', type=int, default=10,
                    help='round number*repeated_times to run in parallel，一个round跑几圈')
parser.add_argument('-rt', '--repeated_times', type=int, default=100,
...
args.model_name = 'rl_pg_vanilla_r12'
args.cuda = 'cuda:0'
args.ps = 'same as base r11 调整了增番与和牌均衡reward'
...
class MahjongEnv:
    ...
    def reshape_reward(self, kind, kind_m, bots, botInd, payoffs, hand_fixed_data_bots_order_round, avg_payoff):
        rewards = []
        ...
        #             r = card & winnercard
        #             r = r.sum() * payoffs[botInd] / 14
        #             if len(rewards) == 0:
        #                 rewards.append(r)
        #             else:
        #                 rewards.append(r - rewards[-1])
        #     elif kind == 'action':
        #         rewards.extend([(payoffs[botInd] * 3 / 14)] * len(kind_m.actions))
        #     else:
        #         rewards.extend([(payoffs[botInd] * 2 / 14)] * len(kind_m.actions))
        ...
        #                 rewards.append(-((-payoffs[botInd] * 3 / 14) ** 0.5))
        #             else:
        #                 rewards.append(0)
        ...
        if self.improve:
            if winner == botInd and payoffs[winner] > 0 and payoffs[winner] > avg_payoff:
                rewards.extend([(payoffs[botInd]/2)**0.5]*len(kind_m.actions))
    ...
    while global_episode_counter.value < args.total_number:
        ...
        with lock:
            ...
            if test and current_round % test_interval == 0:
                ...
                env.print_log('test', args.test_epochs, args.test_epochs, winners, losers, player_scores_test, time_cost)
...
def main():
    ...
    train_thread(args.cuda, shared_model, global_episode_counter,
                 global_training_counter, optimizer, lock, shared_memory)
