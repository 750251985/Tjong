...
parser.add_argument('-o', '--old_path', type=str, default='./models/dl_TIT_3heads_blocks3_fc3_turn_0802-202702.pt', help='path to old model as components')
parser.add_argument('-n', '--new_path', type=str, default='./models/dl_TIT_3heads_blocks3_fc3_turn_0802-202702.pt', help='path to load training model')
...
parser.add_argument('-ji', '--join_interval', type=int, default=4, help='how often to update shared model')
parser.add_argument('--eps_clip', type=float, default=0.2, help='epsilon for p/q clipped')
parser.add_argument('--entropy_weight', type=float, default=1e-4, help='initial entropy loss weight')
parser.add_argument('--entropy_target', type=float, default=1e-5, help='targeted entropy value')
parser.add_argument('--entropy_step', type=float, default=0.001, help='entropy change per step')
parser.add_argument('-tn', '--total_number', type=int, default=2000, help='epochs to run in total，全部epochs，控制停止')
...
parser.add_argument('-rt', '--repeated_times', type=int, default=400,
...
args.model_name = 'rl_pg_vanilla_turn'
args.cuda = 'cuda:1'
...
class MahjongEnv:
    ...
    def run_rounds(self):
        ...
        for i in range(self.round_number):
            for j in range(4):
                if self.train:
                    if self.scores[i][j] != 0 and self.getBotInd(i, j) == 0:
                        round_memory = self.memory_each_rounds[i * 4 + j].get_memory()
                        for kind in round_memory.keys():
                            rewards = self.reshape_reward(kind, round_memory.get(kind), self.bots_orders[i], j, self.scores[i][j])
        ...
        def run_forward(kind):
            ...
            if self.train and model != self.old_model:
                for idx, action, log_prob, card_feat, extra_feat, mask in zip(action_idx, action_numpy, log_probs,
                    ...
                    self.memory_each_rounds[original_order].add_memory(kind, card_feat, extra_feat, mask, action, log_prob)
        kind = 'action'
    ...
    def reshape_reward(self, kind, kind_m, bots, botInd, payoff):
        rewards = []
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                p.data.addcdiv_(exp_avg, denom, value=-step_size[0].item())  # first arg has to be scalar

        return loss
# thread for training
def train_thread(cuda, shared_model, global_episode_counter, global_training_counter, optimizer, lock, shared_memory, winners):
    ...
    while global_episode_counter.value < args.total_number:
        ...
        with lock:
            ...
            if save and current_round % save_interval == 0:
                ...
                print('total rounds: %d, saving model...' % current_round,save_path+'_'+args.model_name+'_'+str(stemp)+".pt")
...
def main():
    ...
    # optimizer = optim.AdamW(shared_model.parameters(), lr=lr)
