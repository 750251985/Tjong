...
parser.add_argument('-lp', '--load_path', type=str, default='./models/dl_TIT_3heads_blocks3_fc3_1682935896.489467.pt',
                    help='from where to load model')
...
args.model_name = 'dl_TIT_3heads_blocks3_fc3_25'
args.cuda = 'cpu'
args.ps = 'base dl_TIT_3heads_blocks3_fc3_turn,add extra feature turnID'
...
cardsWBT = {'W': 0, 'B': 9,
            ...
            # 箭牌
            'J': 31}
cardsTBW = {'T': 0, 'B': 9, 'W': 18,
            # 风
            'F': 27,
            # 箭牌
            'J': 31}

cardsBTW = {'B': 0, 'T': 9, 'W': 18,
            # 风
            'F': 27,
            # 箭牌
            'J': 31}
cardsTWB = {'T': 0, 'W': 9, 'B': 18,
            # 风
            'F': 27,
            # 箭牌
            'J': 31}
cardsWTB = {'W': 0, 'T': 9, 'B': 18,
            # 风
            'F': 27,
            # 箭牌
            'J': 31}
...
class MahjongHandler():
    ...
    def reset(self, initial=False):
                    ...
                    while flag:
                        ...
                        if len(data["card_feats"][start:x]) > 0:
                            probs_temp = self.model(np.array(data['card_feats'][start:x]),
                                                    np.array(data['extra_feats'][start:x]), self.device, kind)
                            start = x
        ...
        if self.save_model and not initial and self.round_count % self.save_interval == 0:
            ...
            print("save_model: " + args.model_name + "_" + str(stemp) + ".pt")
            torch.save(state, args.save_path + args.model_name + "_" + str(stemp) + ".pt",
                       _use_new_zipfile_serialization=False)
            if self.round_count % (self.save_interval * 10) == 0:
                torch.save(self.recorder, args.save_path + args.model_name + "_recorder" + str(stemp),
                           _use_new_zipfile_serialization=False)
                self.recorder = {'play': [], 'action': [], 'chi_gang': []}
        self.round_count += 1
        self.loss = []
    ...
    def step_for_train(self, request=None, response_target=None, fname=None):
        ...
        if self.turnID <= 1:
            ...
            if self.train and response_target is not None and judge_response(available_action_mask):  # 已经排除了pass
                    ...
                    for kind in ['play', 'action', 'chi_gang']:
                        if len(training_data[kind]['card_feats']) >= self.batch_size:
                            ...
                            target_tensor = torch.from_numpy(np.array(data['target'])).to(device=self.device,
                                                                                          dtype=torch.int64)
                            if kind == 'action':
                                losses = F.cross_entropy(probs, target_tensor, weight=self.action_weight).to(
                                    torch.float32)
                            else:
                                losses = F.cross_entropy(probs, target_tensor).to(torch.float32)
    ...
    def step(self, request=None, response_target=None, fname=None):
        ...
        if self.turnID <= 1:
            ...
            action_probs = self.model(np.array([card_feats]),
                                      np.array([extra_feats]), self.device, 'action')
            available_action_mask = torch.from_numpy(available_action_mask).to(self.device)
            available_card_mask = torch.from_numpy(available_card_mask).to(self.device)
            action = make_decision(action_probs * available_action_mask)
            cards = []
            if responses(action) in [responses.CHI, responses.ANGANG, responses.BUGANG]:
                card_probs = self.model(np.array([card_feats]),
                                        np.array([extra_feats]), self.device, 'chi_gang')
                card_ind = make_decision(card_probs * available_card_mask[action])
            ...
            if responses(action) in [responses.PLAY, responses.CHI, responses.PENG]:
                if responses(action) == responses.PLAY:
                    ...
                    card_feats, extra_feats, card_mask = self.simulate_chi_peng(request, responses(action),
                                                                                chi_peng_ind, True)
                    card_feats = card_feats.flatten(order='C')
                    available_card_mask[responses.PLAY.value] = torch.from_numpy(card_mask)

                card_probs = self.model(np.array([card_feats]),
                                        np.array([extra_feats]), self.device, 'play')
                card_ind = make_decision(card_probs * available_card_mask[responses.PLAY.value])
                cards.append(card_ind)
    ...
    def getCardInd(self, cardName):
        ...
        return cards[cardName[0]] + int(cardName[1]) - 1

    def getCardName(self, cardInd):
        num = 1
        while True:
            if cardInd in cards.values():
                break
            num += 1
            cardInd -= 1
        return self.get_key(cards, cardInd)[0] + str(num)
