...
class Config_3heads:
    def __init__(self,
                 ...
                 share_tit_blocks,
                 features_dim_action=8):
        self.algo = algo
        self.patch_dim = patch_dim
        self.num_blocks = num_blocks
        self.features_dim = features_dim
        self.features_dim_action = features_dim_action
...
class TIT_3heads(nn.Module):
    def __init__(self, config):
        super(TIT_3heads, self).__init__()
        self.config = config
        ...
        # Head
        if config.algo in ['vanilla_tit_ppo', 'vanilla_tit_cql']:
            self.ln1_action = nn.LayerNorm(config.embed_dim_outer)
            self.head_action = nn.Sequential(
                nn.Linear(config.embed_dim_outer, config.embed_dim_outer * config.dim_expand_inner),
                config.activation_fn_other(),
                nn.Linear(config.embed_dim_outer * config.dim_expand_inner, config.embed_dim_outer),
                config.activation_fn_other(),
                nn.Linear(config.embed_dim_outer, config.features_dim_action),
                config.activation_fn_other()
            )
            self.ln2_action = nn.LayerNorm(config.features_dim_action)

            self.ln1_chigang = nn.LayerNorm(config.embed_dim_outer)
            self.head_chigang = nn.Sequential(
                nn.Linear(config.embed_dim_outer, config.embed_dim_outer * config.dim_expand_inner),
                config.activation_fn_other(),
                nn.Linear(config.embed_dim_outer * config.dim_expand_inner, config.embed_dim_outer),
                config.activation_fn_other(),
            ...
            self.ln2_chigang = nn.LayerNorm(config.features_dim)

            self.ln1_play = nn.LayerNorm(config.embed_dim_outer)
            ...
            self.ln2_play = nn.LayerNorm(config.features_dim)
    ...
    def forward(self, obs, decide_which):
        ...
        if self.config.algo in ['vanilla_tit_ppo', 'vanilla_tit_cql']:
            ...
            if decide_which == 'action':
                x = self.head_action(self.ln1_action(x))
                x = self.ln2_action(x)  # (B, features_dim_action)
            elif decide_which == 'chi_gang':
                x = self.head_chigang(self.ln1_chigang(x))
                x = self.ln2_chigang(x)  # (B, features_dim)
            elif decide_which == 'play':
                x = self.head_play(self.ln1_play(x))
                x = self.ln2_play(x)  # (B, features_dim)
...
class TitFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space,
        ...
        config = Config_3heads(algo,
                        ...
                        have_position_encoding,
                        share_tit_blocks)
        self.pure_transformer_backbone = TIT_3heads(config)
