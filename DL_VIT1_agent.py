...
class ViT(nn.Module):
    # channels 默认值改为1
    def __init__(self, *, image_size, patch_size, dim, depth, heads, mlp_dim, pool='cls', channels=1,
                 dim_head=64, dropout=0.1, emb_dropout=0):
        super().__init__()
...
parser.add_argument('-lp', '--load_path', type=str, default='./models/dl_model_21677478927.0380445', help='from where to load model')
parser.add_argument('-sp', '--save_path', type=str, default='./models/dl_model_2', help='save model path')
...
parser.add_argument('-si', '--save_interval', type=int, default=500000, help='how often to save')
...
class myModel(nn.Module):
    def __init__(self, num_extra_feats, num_cards, num_actions):
        super(myModel, self).__init__()

        # hidden_channels = [8, 16, 32]
        # hidden_layers_size = [512, 1024]
        # linear_length = hidden_channels[1] * num_cards * card_feat_depth
        params_dict = {'input_shape': (14, num_cards), 'patch_size': (1, num_cards),
                       'input_shape_extra': (num_extra_feats, 1), 'patch_size_extra': (4, 1),
                       'dim': 512, 'depth_cards': 6, 'depth_extra': 3, 'heads_cards': 4, 'heads_extra': 3, 'mlp_dim': 1024, 'fc_dim': 2048, 'dropout': 0.1}
        self.cards_net = ViT(
            image_size=params_dict['input_shape'],
            patch_size=params_dict['patch_size'],
            dim=params_dict['dim'],
            depth=params_dict['depth_cards'],
            heads=params_dict['heads_cards'],
            mlp_dim=params_dict['mlp_dim'],
            dropout=params_dict['dropout'],
        )
        self.extra_feats_net = ViT(
            image_size=params_dict['input_shape_extra'],
            patch_size=params_dict['patch_size_extra'],
            dim=params_dict['dim'],
            depth=params_dict['depth_extra'],
            heads=params_dict['heads_extra'],
            mlp_dim=params_dict['mlp_dim'],
            dropout=params_dict['dropout'],
        )
        self.card_play_decision_head = nn.Sequential(
            nn.LayerNorm(params_dict['mlp_dim']),
            nn.Linear(params_dict['mlp_dim'], params_dict['fc_dim']),
            nn.ReLU(),
            nn.Linear(params_dict['fc_dim'], num_cards),
        )
        self.chi_peng_decision_head = nn.Sequential(
            nn.Linear(params_dict['mlp_dim'], params_dict['fc_dim']),
            nn.ReLU(),
            nn.Linear(params_dict['fc_dim'], num_cards),
        )
        self.action_decision_head = nn.Sequential(
            nn.Linear(params_dict['mlp_dim'], params_dict['fc_dim']),
            nn.ReLU(),
            nn.Linear(params_dict['fc_dim'], num_actions),
        )
    ...
    def forward(self, card_feats, extra_feats, device, decide_which, mask):
        ...
        card_layer = self.cards_net(card_feats)
        extra_feats_tensor = torch.from_numpy(extra_feats).unsqueeze(1).unsqueeze(3).to(torch.float32).to(device)
        extra_layer = self.extra_feats_net(extra_feats_tensor)
        linear_layer = torch.cat((card_layer, extra_layer), dim=1)
        mask_tensor = torch.from_numpy(mask).to(torch.float32).to(device)

        if decide_which == 'play':
            card_probs = self.card_play_decision_head(linear_layer)
            valid_card_play = card_probs * mask_tensor
            return valid_card_play
        elif decide_which == 'action':
            action_probs = self.action_decision_head(linear_layer)
            valid_actions = action_probs * mask_tensor
            return valid_actions
        else:
            card_probs = self.chi_peng_decision_head(linear_layer)
            valid_card_play = card_probs * mask_tensor
            return valid_card_play
...
class MahjongHandler():
    ...
    def reset(self, initial=False):
        ...
        if self.save_model and not initial and self.round_count % self.save_interval == 0:
            ...
            torch.save(state, args.save_path+"_"+str(self.round_count)+"_"+str(time.time()), _use_new_zipfile_serialization=False)
        self.round_count += 1
        self.loss = []
    ...
    def step_for_train(self, request=None, response_target=None, fname=None):
        ...
        if self.turnID <= 1:
            ...
            if self.train and response_target is not None and judge_response(available_action_mask):
                ...
                if available_action_mask.sum() > 1:  # 最少是一个pass动作，>1 代表有动作发生
    ...
    def build_output(self, response, cards_ind):
        ...
        if response == responses.MINGGANG:
            response_name = 'GANG'
        if response == responses.HU and not self.botzone:
...
class agent:
    def __init__(self, model_path, cuda):
        args.cuda = cuda
        self.my_bot = MahjongHandler(train=False, model_path=model_path, load_model=True, save_model=False,
                                     botzone=False)
        self.reset()
...
## nohup python -u DL_TIT_1head_agent.py > log.log 2>&1 &