...
parser.add_argument('-si', '--save_interval', type=int, default=50, help='how often to save')
...
parser.add_argument('-tn', '--total_number', type=int, default=200, help='epochs to run in total，全部epochs，控制停止')
...
parser.add_argument('-rn', '--round_number', type=int, default=200, help='round number*repeated_times to run in parallel，一个round跑几圈')
...
args.model_name = 'rl_ac_vanilla_cuda1_botzone'
args.cuda = 'cuda:1'
    ...
    def print_log(self, type, current_round, print_interval, winners, player_scores, time_cost):
        ...
        print(
            ...
            'Hu {} rounds，Huang-zhuang {} rounds，hu ratio {:.2%}, average rounds to hu: {}, took {:.2f} minutes per 10000 rounds'.format(
                ...
                sum(winners[3:]) / rounds_this_stage,
...
def update(all_memories: dict, model, shared_model, shared_critic, optimizer, optimizer_critic, device):
    model.load_state_dict(shared_model.state_dict())
    model.zero_grad()
    shared_critic.zero_grad()
    optimizer.zero_grad()
    ...
    for kind, memory in all_memories.items():
        ...
        for i in range(batch_number):
            ...
            v_s = shared_critic(outer_token)
            new_probs_, outer_token_ = model(
                card_feats_[start:end],
                extra_feats_[start:end],
                device,
                kind,
                # masks[start:end]
            )
            v_s_ = shared_critic(outer_token_)
            with torch.no_grad():  # td_target has no gradient
                td_target = rewards[start:end] + args.gamma * (1 - dw[start:end]) * v_s_

            # Update actor
            new_probs_dist = Categorical(new_probs)
            new_logprobs = new_probs_dist.log_prob(old_actions[start:end])  # log pi(a|s)
            actor_loss = -args.gamma ** step[start:end] * ((td_target - v_s).detach()) * new_logprobs  # Only calculate the derivative of log_pi
            # Update critic
    ...
    torch.nn.utils.clip_grad_norm_(shared_critic.parameters(), 10)
    ...
    while global_episode_counter.value < args.total_number:
        ...
        with lock:
            ...
            if current_round % train_interval == 0:
                update(shared_memory.get_memory(), model_for_update, shared_model, shared_critic, optimizer, optimizer_critic, env.device)
...
def main():
    ...
    optimizer_critic = SharedAdam(shared_critic.parameters(), lr=lr*5)
...
# nohup python -u AC_vanilla.py > log_AC_vanilla_line_botzone_05091239.log 2>&1 &